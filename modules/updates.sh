...
#   Writes:    apt: git, wget, curl, xinput, zram-tools, tlp
...
#                git/wget/curl/xinput are kept. Note: GIMP is now optional
...
step_updates() {
    ...
    oem_tty_say "--> Installing base tools (git, wget, curl, xinput)…"
    oem_run_log env DEBIAN_FRONTEND=noninteractive apt-get install -y git wget curl xinput
