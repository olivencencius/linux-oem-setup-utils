...
| # | Phase | Action |
|---|---|---|
| 1 | Stop services & user helpers | `systemctl disable --now` for `tlp`, `touchegg`, `keyd`. `pkill` `imwheel` (legacy) / `touchegg --client` / `plank` / `xfdashboard` for `$SUDO_USER`. |
| 2 | `apt purge` everything the toolkit installs | Chrome, Zoom, VLC, GIMP, TLP, ZRAM tools, `imwheel` (legacy), `plank`, touchegg, xfdashboard, wmctrl, xdotool, keyd, language packs (`-pl`, `-gnome-pl`, `-en`, `-gnome-en`), `supertuxkart`, `aisleriot`, `quadrapassel`. Then `apt-get autoremove --purge` + `apt-get autoclean`. |
...
| 11 | `/etc/skel` cleanup | `rm` current autostart entries (`oem-first-run`, `touchegg-client`), legacy `xsettings.xml` if present. Also `rm -f` legacy artefacts (`.imwheelrc`, `imwheel.desktop`, `plank.desktop`, `plank/` tree, `gtk-4.0` symlinks) — no-ops on current revision. `rmdir` empty parents. |
| 12 | Per-user cleanup | For every uid ≥ 1000, plus `$SUDO_USER` (deduped): `rm` `.oem-first-run-done`, autostart entries, `launcher-NNN` dirs with NNN ≥ 100 under `~/.config/xfce4/panel/`, and legacy artefacts (`.imwheelrc`, `plank/`, `gtk-4.0` symlinks). xfconf `/panels/panel-2` subtree removed via `xfconf-query -r -R`. |
