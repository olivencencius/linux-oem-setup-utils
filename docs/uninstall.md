...
| 8 | Wallpaper / first-run script | `rm -rf /usr/share/backgrounds/oem-setup`. `rm /usr/local/bin/oem-first-run.sh`. `rm /usr/share/applications/oem-workspace-overview.desktop`. Also cleans legacy artefacts from earlier revisions: `rm /etc/dconf/db/local.d/00-plank`, `dconf update`. |
...
- **`zenity`, `policykit-1`, `oem-config-gtk`** — installed by
  `step_powerwash`, but commonly already present on Mint OEM images,
  and removing them might break the system. They are left in place.
...
- **User accounts** — `step_uninstall` removes toolkit *artefacts*
  ...
  This is the deliberate difference from Powerwash: uninstall is for
  "undo the deployment", Powerwash is for "give the machine back to
  factory".
...
# 4. Theme reset?
xfconf-query -c xsettings -p /Net/ThemeName                                    # not 'ChromeOS'

# 5. State cleared?
ls /var/lib/oem-setup/state/                                                   # should not exist

# 6. Powerwash tool gone?
pkaction --action-id org.linuxoem.powerwash.arm 2>&1                           # 'not registered'
```
