...
| 10 | Regional | `restore_or_skip /etc/default/keyboard` (only when a legacy snapshot exists) or **`sed`** `XKBLAYOUT="us"`. `setupcon`. `localectl set-locale LANG=en_US.UTF-8`. `timedatectl set-timezone UTC` (uninstall resets timezone regardless of installer). |
