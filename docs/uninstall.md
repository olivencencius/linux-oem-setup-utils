...
| 2 | `apt purge` everything the toolkit installs | Chrome, Zoom, `oem-config` + `oem-config-gtk`, VLC, GIMP, TLP, ZRAM tools, `imwheel` (legacy), `plank`, **`touchegg` (legacy apt package**, if installed), **xfdashboard**, **rofi**, wmctrl, xdotool, keyd, language packs (`-pl`, `-gnome-pl`, `-en`, `-gnome-en`), `supertuxkart`, `aisleriot`, `quadrapassel`. **`libinput-gestures`** is removed via **`libinput-gestures-setup uninstall`**, not `apt purge`. Then `apt-get autoremove --purge` + `apt-get autoclean`. |
...
| 11 | `/etc/skel` cleanup | `rm` current autostart entries (`oem-first-run`, `touchegg-client` legacy, `libinput-gestures` if present), `Desktop/oem-prepare-shipping.desktop`, legacy `xsettings.xml` if present. Also `rm -f` legacy artefacts (`.imwheelrc`, `imwheel.desktop`, `plank.desktop`, `plank/` tree, `gtk-4.0` symlinks) — no-ops on current revision. `rmdir` empty parents. |
